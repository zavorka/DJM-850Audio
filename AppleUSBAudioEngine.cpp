...
OSDefineMetaClassAndStructors(DJM03AudioEngine, IOAudioEngine)

#pragma mark -IOKit Routines-

void DJM03AudioEngine::free () {
	debugIOLog ("+ DJM03AudioEngine[%p]::free ()", this);
...
bool DJM03AudioEngine::init ( OSArray * streamInterfaceNumberArray ) {
	Boolean			result = false;

	debugIOLog("+ DJM03AudioEngine[%p]::init ()", this);
...
bool DJM03AudioEngine::requestTerminate (IOService * provider, IOOptionBits options) {
    bool result = mUSBAudioDevice == provider;
	debugIOLog ("- DJM03AudioEngine[%p]::requestTerminate (%p, %x) = %d", this, provider, options, result);
	return result;
}

bool DJM03AudioEngine::start (IOService * provider, IOAudioDevice * device) {
	bool								resultCode;

	debugIOLog ("+ DJM03AudioEngine[%p]::start (%p)", this, provider);
	resultCode = FALSE;		
	
	FailIf (NULL == device, Exit);

	mUSBAudioDevice = OSDynamicCast (DJM03AudioDevice, device);
	//	<rdar://62775111>	Retain a reference to the DJM03AudioDevice object here so that it doesn't goes away
...
Exit:    
	...
	debugIOLog ("- DJM03AudioEngine[%p]::start (%p) = %d", this, provider, resultCode);
	return resultCode;
}
    
void DJM03AudioEngine::stop (IOService * provider) {
    debugIOLog("+ DJM03AudioEngine[%p]::stop (%p)", this, provider);
	...
	debugIOLog ( "- DJM03AudioEngine[%p]::stop (%p) - rc=%ld", this, provider, getRetainCount() );
}

bool DJM03AudioEngine::terminate (IOOptionBits options) {
	debugIOLog ("DJM03AudioEngine[%p]::terminate ()", this);
    return super::terminate (options);
}

bool DJM03AudioEngine::matchPropertyTable(OSDictionary * table, SInt32 *score)
{
	return (super::matchPropertyTable (table, score) &&
            compareProperty (table, kIDVendorString) &&
...
void DJM03AudioEngine::registerService(IOOptionBits options)
{
	debugIOLog ("+ DJM03AudioEngine[%p]::registerService ( 0x%lx )", this, options);
...
void DJM03AudioEngine::openStreamInterfaces () {

	FailIf ( NULL == mIOAudioStreamArray, Exit );
	
	for ( UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++ )
	{
		DJM03AudioStream * audioStream = OSDynamicCast (DJM03AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
...
void DJM03AudioEngine::closeStreamInterfaces () {
...
IOReturn DJM03AudioEngine::clipOutputSamples (const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream) {
	IOReturn				result;
	DJM03AudioStream *		usbAudioStream;			

	result = kIOReturnError;
	
	usbAudioStream = OSDynamicCast ( DJM03AudioStream, audioStream );
	FailIf ( NULL == usbAudioStream, Exit );
	
	usbAudioStream->queueOutputFrames ();
    
	if (TRUE == streamFormat->fIsMixable) 
	{
		if (usbAudioStream->mPlugin)
		{
			usbAudioStream->mPlugin->pluginProcess ((Float32*)mixBuf + (firstSampleFrame * streamFormat->fNumChannels), numSampleFrames, streamFormat->fNumChannels);
		}
		if (mPlugin) 
		{
			if (usbAudioStream == mMainOutputStream)
			{
		...
		result = clipAudioToOutputStream (mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
			...
			if (!mHaveClipped)
			{
				...
				// debugIOLog ("! DJM03AudioEngine::clipOutputSamples() - Sample frame %lu queued for USB frame %llu (current frame = %llu)", firstSampleFrame, usbAudioStream->getQueuedFrameForSample (firstSampleFrame), mUSBAudioDevice->getUSBFrameNumber());
			}
		...
		if (mUHCISupport)
		{
			...
			UInt16 alternateFrameSize = usbAudioStream->getAlternateFrameSize ();
			...
			if ( start < alternateFrameSize )
			{
				...
				memcpy( &( ( ( char * )sampleBuf )[ usbAudioStream->mSampleBufferSize + start ] ), & ( ( ( char * )sampleBuf )[ start ] ), size );
				#if DEBUGUHCI
				debugIOLog( "? DJM03AudioEngine::clipOutputSamples () - firstSampleFrame = %d. Just copied %d bytes from %d to %d \n", firstSampleFrame, size, start, start + usbAudioStream->mSampleBufferSize );			
				#endif
			}
		} // UHCI support
	} 
...
IOReturn DJM03AudioEngine::controlledFormatChange (IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
	IOReturn							result;
	...
	debugIOLog ("+ DJM03AudioEngine[%p]::controlledFormatChange (%p, %p, %p)", this, audioStream, newFormat, newSampleRate);
	...
	result = usbAudioStream->controlledFormatChange ( newFormat, newSampleRate );
	FailIf ( kIOReturnSuccess != result, Exit );

	for (UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++)
	{
		DJM03AudioStream * stream = OSDynamicCast (DJM03AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
		FailIf ( NULL == stream, Exit );
	
		if ( stream != usbAudioStream )
		{
	...
	if (newFormat->fNumChannels != audioStream->format.fNumChannels) 
	{
		...
		debugIOLog ("? DJM03AudioEngine[%p]::controlledFormatChange () - Need to adjust channel controls (cur = %d, new = %d)", this, audioStream->format.fNumChannels, newFormat->fNumChannels);
		
		if (kIOAudioStreamDirectionOutput == usbAudioStream->getDirection ())
		{
			// check for mono mode
	...
	if (newSampleRate) 
	{
		debugIOLog ("? DJM03AudioEngine[%p]::controlledFormatChange () - Changing sampling rate to %d", this, newSampleRate->whole);
	...
	if ((usbAudioStream == mMainOutputStream) || ((usbAudioStream == mMainInputStream) && (NULL == mMainOutputStream)))
	{
		if (mPlugin) 
		{
	...
	debugIOLog("? DJM03AudioEngine[%p]::controlledFormatChange () - New numSamplesInBuffer = %d", this, numSamplesInBuffer );
	...
	if ( TRUE == needToChangeChannels )
	{
		...
		if ( IP_VERSION_02_00 == mUSBAudioDevice->mControlInterface->GetInterfaceProtocol() )
		{
			...
			doClockSelectorSetup ( usbAudioStream->mInterfaceNumber, usbAudioStream->mAlternateSettingID, mCurSampleRate.whole );
		}	
		completeConfigurationChange ();
	}

	debugIOLog ("? DJM03AudioEngine[%p]::controlledFormatChange () - Called setNumSampleFramesPerBuffer with %d", this, usbAudioStream->mSampleBufferSize / (usbAudioStream->mSampleSize ? usbAudioStream->mSampleSize : 1));
	debugIOLog ("? DJM03AudioEngine[%p]::controlledFormatChange () - newFormat->fNumChannels = %d, newFormat->fBitWidth %d", this, newFormat->fNumChannels, newFormat->fBitWidth);

	result = kIOReturnSuccess;

Exit:
	debugIOLog ("- DJM03AudioEngine[%p]::controlledFormatChange () = 0x%x", this, result);
    return result;
...
IOReturn DJM03AudioEngine::convertInputSamples (const void *sampleBuf,
                                                                void *destBuf,
...
#if DEBUGCONVERT
	debugIOLog ("+ DJM03AudioEngine::convertInputSamples (%p, %p, %lu, %lu, %p, %p)", sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat, audioStream);
#endif
	...
	usbAudioStream = OSDynamicCast ( DJM03AudioStream, audioStream );
	FailWithAction ( NULL == usbAudioStream, result = kIOReturnError, Exit );
	
	//	if (!usbAudioStream->mHaveTakenFirstTimeStamp)
	//	{
	//		debugIOLog ("! DJM03AudioEngine::convertInputSamples () - called before first time stamp!");
	//	}
	
	// <rdar://7298196> Only perform the conversion if the audio engine is running.
	if ( mUSBStreamRunning )
	{
		usbAudioStream->queueInputFrames (); 
		
		// <rdar://problem/7378275>
		if (usbAudioStream->mCoalescenceMutex)
		{
			IORecursiveLockLock (usbAudioStream->mCoalescenceMutex);
		}
		...
		if (usbAudioStream->mBufferOffset + 1 > usbAudioStream->getSampleBufferSize ()) 
		{
			windowStartByte = 0;
		} 
		else 
		{
			windowStartByte = usbAudioStream->mBufferOffset + 1;
		}
		windowEndByte = windowStartByte + (usbAudioStream->mNumUSBFrameListsToQueue * usbAudioStream->mReadUSBFrameListSize);
		if (windowEndByte > usbAudioStream->getSampleBufferSize ()) 
		{
			windowEndByte -= usbAudioStream->getSampleBufferSize ();
		}
		...
		{
			// debugIOLog ("%ld, %ld, %ld, %ld, %ld, %ld, %ld", firstSampleFrame * 4, numSampleFrames, lastSampleByte, usbAudioStream->mCurrentFrameList, usbAudioStream->mBufferOffset, windowStartByte, windowEndByte);
			if (usbAudioStream->mBufferOffset < lastSampleByte) 
			{
				// [rdar://5355808] Keep track of sample data underruns.
				coalescenceErrorCode = usbAudioStream->CoalesceInputSamples (lastSampleByte - usbAudioStream->mBufferOffset, NULL);
#if DEBUGLOADING
				debugIOLog ("! DJM03AudioEngine::convertInputSamples () - Coalesce from convert %d bytes", lastSampleByte - usbAudioStream->mBufferOffset);
#endif
			} 
			else 
			{
				// Have to wrap around the buffer.
				UInt32		numBytesToCoalesce = usbAudioStream->getSampleBufferSize () - usbAudioStream->mBufferOffset + lastSampleByte;
				// [rdar://5355808] Keep track of sample data underruns.
				coalescenceErrorCode = usbAudioStream->CoalesceInputSamples (numBytesToCoalesce, NULL);
#if DEBUGLOADING
				debugIOLog ("! DJM03AudioEngine::convertInputSamples () - Coalesce from convert %d bytes (wrapping)", numBytesToCoalesce);
#endif
			}
		}
		...
		if (usbAudioStream->mCoalescenceMutex)
		{
			IORecursiveLockUnlock (usbAudioStream->mCoalescenceMutex);
		}
		result = convertFromAudioInputStream_NoWrap (sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat);
		
		if (usbAudioStream->mPlugin)
		{
			usbAudioStream->mPlugin->pluginProcessInput ((float *)destBuf, numSampleFrames, streamFormat->fNumChannels);
		}
		if (mPlugin) 
		{
			if (usbAudioStream == mMainInputStream)
			{
...
Exit:	
	...
	{
		debugIOLog ( "! DJM03AudioEngine::convertInputSamples () = 0x%x", result );
	}
	
	return result;
}

IOReturn DJM03AudioEngine::eraseOutputSamples(const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream)
{	
	...
	if (mUHCISupport)
	{
		DJM03AudioStream * usbAudioStream = OSDynamicCast ( DJM03AudioStream, audioStream );
		FailIf ( NULL == usbAudioStream, Exit );
	
		if ( usbAudioStream->getDirection () == kIOAudioStreamDirectionOutput )
		{
			UInt16 alternateFrameSize = usbAudioStream->getAlternateFrameSize ();
			UInt32 start = firstSampleFrame * streamFormat->fNumChannels * (streamFormat->fBitWidth / 8); // first byte index of first sample that is being clipped
			if ( start < alternateFrameSize )
			{
				UInt32 size = alternateFrameSize - start;
				bzero( &( ( ( char * )sampleBuf )[ usbAudioStream->mSampleBufferSize + start ] ), size );
			}
		}
	}
...
UInt32 DJM03AudioEngine::getCurrentSampleFrame () {
...
IOReturn DJM03AudioEngine::GetDefaultSampleRate (IOAudioSampleRate * sampleRate) {
	...
	debugIOLog ("+ DJM03AudioEngine[%p]::GetDefaultSampleRate ()", this);
	result = kIOReturnError;
	...
	for ( streamIndex = 0; streamIndex < mStreamInterfaceNumberArray->getCount (); streamIndex++ )
	{
		...
		result = configDictionary->getAltSettingWithSettings (&newAltSettingID, interfaceNumber, kChannelDepth_STEREO, kBitDepth_24bits, newSampleRate.whole);
		if (		( kIOReturnSuccess == result )
	...
	if (!foundSampleRate)
	{
		for ( streamIndex = 0; streamIndex < mStreamInterfaceNumberArray->getCount (); streamIndex++ )
		{
			...
			result = configDictionary->getAltSettingWithSettings (&newAltSettingID, interfaceNumber, kChannelDepth_MONO, kBitDepth_24bits, newSampleRate.whole);
			if (		( kIOReturnSuccess == result )
					&&	( kIOReturnSuccess == configDictionary->getFormat( &format, interfaceNumber, newAltSettingID ) )
					&&	( PCM == format ) )
			{
	...
	if (!foundSampleRate)
	{
		for ( streamIndex = 0; streamIndex < mStreamInterfaceNumberArray->getCount (); streamIndex++ )
		{
			...
			// Don't have a mono or stereo 16-bit 44.1kHz interface, so try for a stereo 24-bit interface with any sample rate
			if (		( kIOReturnSuccess == (result = configDictionary->getAltSettingWithSettings (&newAltSettingID, interfaceNumber, kChannelDepth_STEREO, kBitDepth_24bits ) ) )
					&&	( kIOReturnSuccess == configDictionary->getFormat( &format, interfaceNumber, newAltSettingID )
					&&  ( PCM == format ) ) )
			{
	...
	if (!foundSampleRate)
	{
		for ( streamIndex = 0; streamIndex < mStreamInterfaceNumberArray->getCount (); streamIndex++ )
		{
			...
			if (		( kIOReturnSuccess == (result = configDictionary->getAltSettingWithSettings (&newAltSettingID, interfaceNumber, kChannelDepth_MONO, kBitDepth_24bits ) ) )
					&&	( kIOReturnSuccess == configDictionary->getFormat( &format, interfaceNumber, newAltSettingID )
					&&  ( PCM == format ) ) )
			{
	...
	if (!foundSampleRate)
	{
		...
		debugIOLog ("? DJM03AudioEngine[%p]::GetDefaultSampleRate () - Taking first available alternate setting (%d)", this, newAltSettingID);
	...
	debugIOLog ("? DJM03AudioEngine[%p]::GetDefaultSampleRate () - Default sample rate is %d", this, newSampleRate.whole);
	debugIOLog ("? DJM03AudioEngine[%p]::GetDefaultSampleRate () - Default alternate setting ID is %d", this, newAltSettingID);
	FailIf (0 == newSampleRate.whole, Exit);
	*sampleRate = newSampleRate;
	result = kIOReturnSuccess;
	
Exit:
	debugIOLog ("- DJM03AudioEngine[%p]::GetDefaultSampleRate (%lu) = 0x%x", this, sampleRate->whole, result);
	return result;
}

OSString * DJM03AudioEngine::getGlobalUniqueID () {
    char *						uniqueIDStr;
    OSString *					localID;
    OSString *					uniqueID;
	...
	uniqueIDSize += strlen ("DJM03AudioEngine");
	...
	if ( nameObject )
	{
		...
		{
			debugIOLog ( "? DJM03AudioEngine[%p]::getGlobalUniqueID () - Retrieved vendor name %s from registry", this, nameString->getCStringNoCopy () );
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM03AudioEngine[%p]::getGlobalUniqueID () - Retrieved product name %s from registry", this, nameString->getCStringNoCopy () );
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "? DJM03AudioEngine[%p]::getGlobalUniqueID () - Retrieved serial number %s from registry", this, nameString->getCStringNoCopy () );
	...
	else 
	{
		...
		debugIOLog ("? DJM03AudioEngine[%p]::getGlobalUniqueID () - Device has a serial number = %s", this, serialNumberString);
	...
	for ( UInt32 index = 0; index < mStreamInterfaceNumberArray->getCount (); index++ )
	{
		...
		{
			interfaceNumber = streamInterfaceNumber->unsigned8BitValue ();
			sprintf (interfaceNumberString, "%s%s%d", interfaceNumberString, (index == 0) ? "" : ",", interfaceNumber);
	...
	if (NULL != uniqueIDStr) 
	{
		...
		{
			sprintf (uniqueIDStr, "DJM03AudioEngine:%s:%s:%s", manufacturerString, productString, locationIDString);
		} 
		else 
		{
			sprintf (uniqueIDStr, "DJM03AudioEngine:%s:%s:%s", manufacturerString, productString, serialNumberString);
		}
		...
		debugIOLog ("DJM03AudioEngine[%p]::getGlobalUniqueID () - getGlobalUniqueID = %s", this, uniqueIDStr);
...
//--------------------------------------------------------------------------------
bool DJM03AudioEngine::initHardware (IOService *provider) {
...
//	UInt32								numTriesForStreamPropertiesReady;	//  <rdar://problem/6686515>
	...
	for ( UInt32 streamIndex = 0; streamIndex < mStreamInterfaceNumberArray->getCount (); streamIndex++ )
	{
		...
		IOUSBInterface * streamInterface = mUSBAudioDevice->getUSBInterface (0);//streamInterfaceNumber->unsigned8BitValue ());
...
/*		propertiesReadyObject = NULL;
		...
		{
			propertiesReadyObject = streamInterface->getProperty ( "DJM03AudioStreamPropertiesReady" );
			if ( propertiesReadyObject )
			{
				debugIOLog ( "! DJM03AudioStream[%p]::initHardware () - DJM03AudioStreamPropertiesReady found for stream #%d", this, streamInterfaceNumber->unsigned8BitValue () );
				break;
			}
			debugIOLog ( "! DJM03AudioStream[%p]::initHardware () - DJM03AudioStreamPropertiesReady NOT found for stream #%d, sleeping 10 ms...", this, streamInterfaceNumber->unsigned8BitValue () );
			IOSleep ( 10 );
			FailIf ( mUSBAudioDevice->isInactive (), Exit );
		}
*/				
		// Instantiate one AppleUSBStream per IOUSBInterface.
		DJM03AudioStream  * audioStream = OSTypeAlloc ( DJM03AudioStream );
		FailIf (NULL == audioStream, Exit);
		...
		if ( mStreamInterfaceNumberArray->getCount () > 1 )
		{
			...
			if ( nameObject )
			{
				if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
				{
					debugIOLog ( "! DJM03AudioStream[%p]::initHardware () - Retrieved product name %s", this, nameString->getCStringNoCopy () );
					streamDescription = (char *)nameString->getCStringNoCopy();
				} 
			}
		}
		
		FailWithAction (!audioStream->initWithAudioEngine (mUSBAudioDevice, this, streamInterfaceNumber->unsigned8BitValue (), sampleRate, streamDescription), audioStream->release(), Exit);
//		FailWithAction (!audioStream->initWithAudioEngine (mUSBAudioDevice, this, streamInterface, sampleRate, streamDescription), audioStream->release(), Exit);
	...
	if ( ( NULL != mMainOutputStream ) && ( kAdaptiveSyncType == mMainOutputStream->getSyncType () ) )
	{
		...
		debugIOLog ("? DJM03AudioEngine[%p]::initHardware (%p) - Main output stream (%p) is adaptive and designated as master", this, provider, mMainOutputStream);
...
//	masterStream = mMainInputStream;
//	syncOutputCompensation = true;
//	syncInputCompensation = true;

	FailIf ( NULL == masterStream, Exit );
	
	masterStream->setMasterStreamMode ( true );
		
	debugIOLog ("? DJM03AudioEngine[%p]::initHardware (%p) - Compensate for output synchronization: %d", this, provider, syncOutputCompensation);
	...
	debugIOLog ("? DJM03AudioEngine[%p]::initHardware (%p) - Compensate for input synchronization: %d", this, provider, syncInputCompensation);
	...
	if ( nameObject )
	{
		if ( NULL != ( nameString = OSDynamicCast ( OSString, nameObject ) ) )
		{
			debugIOLog ( "! DJM03AudioEngine[%p]::initHardware () - Retrieved product name %s", this, nameString->getCStringNoCopy () );
	...
	if (NULL != coreAudioPluginPathString)
	{
		debugIOLog ( "! DJM03AudioEngine[%p]::initHardware () - Retrieved CoreAudio plugin path %s", this, coreAudioPluginPathString->getCStringNoCopy () );
...
Exit:
	// <rdar://problem/6576824> DJM03Audio fails to unload properly
	if ( !resultBool )
	{
    ...
    debugIOLog("- DJM03AudioEngine[%p]::initHardware(%p), resultCode = %x, resultBool = %d", this, provider, resultCode, resultBool);
    return resultBool;
}

//--------------------------------------------------------------------------------
// <rdar://7558825> Get the system clock domain
UInt32 DJM03AudioEngine::getSystemClockDomain ( void )
{
...
OSString * DJM03AudioEngine::getChannelNameString ( UInt8 unitID, UInt8 channelNum )
{
	DJM03ConfigurationDictionary *		configDictionary = NULL;
	...
	debugIOLog ("? DJM03AudioEngine[%p]::getChannelNameString (%d, %d) - Audio cluster descriptor { %d, %d, %d }", this, unitID, channelNum, clusterDescriptor.bNrChannels, clusterDescriptor.bmChannelConfig, clusterDescriptor.iChannelNames);
	...
	if ( ( numPreDefinedChannels != channelNum ) && ( channelNum <= clusterDescriptor.bNrChannels ) )
	{
		...
		if ( 0 != clusterDescriptor.iChannelNames )
		{
			...
			debugIOLog ("? DJM03AudioDevice::getChannelNameString (%d, %d) - stringIndex = %d, stringBuffer = %s, result = 0%x", unitID, channelNum, clusterDescriptor.iChannelNames + channelNum - 1 - numPreDefinedChannels, stringBuffer, result);
		}
	}
	
	if ( kIOReturnSuccess == result )
	{
		debugIOLog ("? DJM03AudioDevice::getChannelNameString () - terminalID = %d, channelNum = %d, stringBuffer = %s", unitID, channelNum, stringBuffer);
...
void DJM03AudioEngine::updateChannelNames ( OSArray* thePath, UInt8 interfaceNum, UInt8 altSettingNum ) {
    ...
    debugIOLog ("+ DJM03AudioEngine[%p]::updateChannelNames (%d, %d)", this, interfaceNum, altSettingNum);
	...
	for ( UInt32 streamIndex = 0; streamIndex < mIOAudioStreamArray->getCount (); streamIndex++ )
	{
		...
		{
			debugIOLog ("? DJM03AudioEngine[%p]::updateChannelNames (%d, %d) - Found audio stream = %p", this, interfaceNum, altSettingNum, audioStream);
			...
			debugIOLog ("? DJM03AudioEngine[%p]::updateChannelNames (%d, %d, %d) - Audio cluster descriptor { %d, %d, %d }", this, unitID, interfaceNum, altSettingNum, clusterDescriptor.bNrChannels, clusterDescriptor.bmChannelConfig, clusterDescriptor.iChannelNames);
			...
			for ( UInt8 channelIndex = 0; channelIndex < clusterDescriptor.bNrChannels; channelIndex++ )
			{
				...
				if ( NULL != nameString )
				{
					...
					if ( kIOAudioStreamDirectionOutput == audioStream->getDirection () )
					{
						sprintf ( keyString, kIOAudioEngineFullChannelNameKeyOutputFormat, audioStream->getStartingChannelID () + channelIndex );
					}
					else
					{
						sprintf ( keyString, kIOAudioEngineFullChannelNameKeyInputFormat, audioStream->getStartingChannelID () + channelIndex );
					}
					
					debugIOLog ("? DJM03AudioEngine[%p]::updateChannelNames (%d, %d, %d) - Setting %s to %s", this, unitID, interfaceNum, altSettingNum, keyString, nameString->getCStringNoCopy());
...
Exit:
    debugIOLog ("- DJM03AudioEngine[%p]::updateChannelNames (%d, %d, %d)", this, unitID, interfaceNum, altSettingNum);
	return;
}

char * DJM03AudioEngine::ChannelConfigString (UInt8 channel) 
{
...
void DJM03AudioEngine::registerPlugin (DJM03AudioPlugin * thePlugin) {
	mPlugin = thePlugin;
...
void DJM03AudioEngine::pluginLoaded (DJM03AudioEngine * usbAudioEngineObject) {
	IOReturn							result;
...
IOReturn DJM03AudioEngine::pluginDeviceRequest (IOUSBDevRequest * request, IOUSBCompletion * completion) {
	IOReturn						result;
...
void DJM03AudioEngine::pluginSetConfigurationApp (const char * bundleID) {
	if (bundleID) 
	{
...
IOReturn DJM03AudioEngine::performAudioEngineStart () {
    IOReturn				resultCode;
	UInt32					usbFramesToDelay = 0;
	UInt32					lockDelayFrames = 0;
	UInt64					currentUSBFrame = 0;
	UInt32					streamIndex;
	DJM03AudioStream *		audioStream;
	

    debugIOLog ("+ DJM03AudioEngine[%p]::performAudioEngineStart ()", this);
	...
	if (mUSBAudioDevice->mAnchorTime.n == 0)
	{
		...
		debugIOLog ("! DJM03AudioEngine[%p]::performAudioEngineStart () - Getting an anchor for the first timestamp.", this);
	...
	{
		audioStream = OSDynamicCast (DJM03AudioStream, mIOAudioStreamArray->getObject (streamIndex) );
	...
	if (!mUSBStreamRunning) 
	{
		...
		// Added an offset to compensate for the time needed for some of the operation in DJM03AudioStream::startUSBStream();
...
Exit:
	...
	{
		debugIOLog ("! DJM03AudioEngine[%p]::performAudioEngineStart () - NOT started, error = 0x%x", this, resultCode);
				...
				&&	( ! mUSBAudioDevice->recoveryRequested ()))
		{
			...
			if (mUSBAudioDevice)
			{
				debugIOLog ("! DJM03AudioEngine[%p]::performAudioEngineStart () - Device not responding! Requesting a recovery attempt.");
...
IOReturn DJM03AudioEngine::performAudioEngineStop() {
	UInt32					streamIndex;
	DJM03AudioStream *		audioStream;

    debugIOLog("+ DJM03AudioEngine[%p]::performAudioEngineStop ()", this);
    ...
    debugIOLog("? DJM03AudioEngine[%p]::performAudioEngineStop() - stopped", this);
...
IOReturn DJM03AudioEngine::performFormatChange (IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate) {
    IOReturn							result;
	UInt32								controllerResult;
	bool								streamIsRunning = false;

	debugIOLog ("+ DJM03AudioEngine[%p]::performFormatChange (%p, %p, %p)", this, audioStream, newFormat, newSampleRate);
	result = kIOReturnError;
	FailIf (NULL == mUSBAudioDevice, Exit);
	...
	switch (controllerResult)
	{
		...
		case kAUAFormatChangeForced:
			debugIOLog ("? DJM03AudioEngine[%p]::performFormatChange () - This request was forced.");
			result = kIOReturnSuccess;
			break;
		case kAUAFormatChangeForceFailure:
			debugIOLog ("! DJM03AudioEngine[%p]::performFormatChange () - Force of this request was attempted but failed.");
			result = kIOReturnSuccess;
			break;
		case kAUAFormatChangeError:
		default:
			debugIOLog ("! DJM03AudioEngine[%p]::performFormatChange () - Error encountered.");
			result = kIOReturnError;
	}
...
Exit:
	...
	debugIOLog ("- DJM03AudioEngine[%p]::performFormatChange () = 0x%x", this, result);
    return result;
}

void DJM03AudioEngine::resetClipPosition (IOAudioStream *audioStream, UInt32 clipSampleFrame) {
	DJM03AudioStream *		usbAudioStream;			
	...
	if (usbAudioStream->mPlugin)
	{
		usbAudioStream->mPlugin->pluginReset ();
	}
	if (mPlugin) 
	{
		mPlugin->pluginReset ();
...
void DJM03AudioEngine::takeTimeStamp (bool incrementLoopCount, UInt64 *timestamp)
{
	#if LOGTIMESTAMPS
	UInt64	time_nanos;
	...
	if (true)
	{
		debugIOLog ("? DJM03AudioEngine[%p]::takeTimeStamp (%d, %p) = %llu ns", this, timestamp, time_nanos);
	}
	#endif
	super::takeTimeStamp (incrementLoopCount, (AbsoluteTime *)timestamp);
}

bool DJM03AudioEngine::willTerminate (IOService * provider, IOOptionBits options) {
	debugIOLog ("+ DJM03AudioEngine[%p]::willTerminate (%p)", this, provider);	
	...
	debugIOLog ("- DJM03AudioEngine[%p]::willTerminate ()", this);

	return super::willTerminate (provider, options);
}

// <rdar://problem/6021475> DJM03Audio: Status Interrupt Endpoint support
OSSet * DJM03AudioEngine::copyDefaultAudioControls ( void )
{
...
//	<rdar://5811247>
IOReturn DJM03AudioEngine::doClockSelectorSetup (UInt8 interfaceNum, UInt8 altSettingNum, UInt32 sampleRate) {
	...
	debugIOLog ("+ DJM03AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d )", this, interfaceNum, altSettingNum, sampleRate);
	...
	{
		debugIOLog ("? DJM03AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - clockSelectorID = %d, clockPathIndex = %d", this, interfaceNum, altSettingNum, sampleRate, clockSelectorID, clockPathIndex);
		...
		for ( UInt32 pathIndex = 0; pathIndex < clockPathGroup->getCount (); pathIndex++ )
		{
			...
			debugIOLog ("? DJM03AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - %d: clockID = %d, subType = %d", this, interfaceNum, altSettingNum, sampleRate, pathIndex, clockID, subType);
	...
	debugIOLog ("? DJM03AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) - Should poll = %d", this, interfaceNum, altSettingNum, sampleRate, mShouldPollClockStatus);
	#endif // POLLCLOCKSTATUS
	
	result = kIOReturnSuccess;
Exit:

	debugIOLog ("- DJM03AudioEngine[%p]::doClockSelectorSetup( 0x%x, 0x%x, %d ) = 0x%x", this, interfaceNum, altSettingNum, sampleRate, result);

	return result;
}

//	<rdar://5811247>
IOReturn DJM03AudioEngine::controlChangedHandler (OSObject * target, IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn						result;
	DJM03AudioEngine *				self;

	result = kIOReturnError;

	self = OSDynamicCast (DJM03AudioEngine, target);
...
//	<rdar://5811247>
IOReturn DJM03AudioEngine::protectedControlChangedHandler (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
//	<rdar://5811247>
IOReturn DJM03AudioEngine::doClockSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	UInt8						clockSourceID;
	...
	debugIOLog ("+ DJM03AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	...
	if ( oldValue != newValue )
	{
		...
		debugIOLog ("? DJM03AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x ) - result = 0x%x, sample rate = %d, clockValidity = %d", this, audioControl, oldValue, newValue, result, newSampleRate.whole, clockValidity);
	...
	debugIOLog ("- DJM03AudioEngine[%p]::doClockSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
...
//	<rdar://5811247>
IOReturn DJM03AudioEngine::republishAvailableFormats () {
	DJM03ConfigurationDictionary *		configDictionary = NULL;
...
bool DJM03AudioEngine::determineMacSyncMode ( UInt8 clockID )
{
	...
	DJM03ConfigurationDictionary *		configDictionary = NULL;
	UInt8								controlInterfaceNum;
	DJM03AudioStream *					masterStream = NULL;	// <rdar://7558825>
	bool								macSyncMode = false;

	debugIOLog ( "+ DJM03AudioEngine[%p]::determineMacSyncMode( %d )", this, clockID );
...
Exit:
	debugIOLog ( "- DJM03AudioEngine[%p]::determineMacSyncMode( %d ) = %d", this, clockID, macSyncMode );
	
	return macSyncMode;
}

//	<rdar://6945472>
IOAudioSampleRate DJM03AudioEngine::getCurrentClockPathSampleRate ( void )
{
...
//	<rdar://5811247>
void DJM03AudioEngine::updateClockStatus ( UInt8 clockID )
{
	UInt32				clockRate;
		...
		if ( kIOReturnSuccess == mUSBAudioDevice->getCurClockSourceSamplingFrequency ( clockID, &clockRate, &clockValidity ) )
		{
			if ( NULL != mClockSelectorControl )
			{
								...
								if ( NULL != selectionNumber )
								{
									...
									if ( selection != currentValue )
									{
										...
										if ( kIOReturnSuccess == mUSBAudioDevice->getCurClockSourceSamplingFrequency ( clockID, &clockRate, &clockValidity ) && ( clockValidity ) )
										{
											...
											debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - Switch over to selection = 0x%x", this, selection );
				...
				else 
				{
					...
					debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - Current sample rate = %u, new sample rate = %u", this, mCurSampleRate.whole, sampleRate.whole );
					...
					if ( mCurSampleRate.whole != sampleRate.whole )
					{
						...
						debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - Re-publishing audio formats", this );

						// Re-publish all the streams supported sample rates & formats.
						republishAvailableFormats ();
						
						debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - Performing format - sample rate changes", this );

						// Do the sample rate change.
						FailMessage ( kIOReturnSuccess != performFormatChange ( NULL, NULL, &sampleRate ) );
						
						debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - Informing CoreAudio of the sample rate change", this );
			...
			{
				debugIOLog ( "? DJM03AudioDevice[%p]::updateClockStatus () - No clock selector present - update audio formats if necessary", this );
...
//	<rdar://5811247>
void DJM03AudioEngine::runPolledTask () {
