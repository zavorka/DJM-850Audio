...
#ifndef _DJM03AudioDEVICE_H
#define _DJM03AudioDEVICE_H
...
class DJM03AudioDevice : public IOAudioDevice {
    OSDeclareDefaultStructors (DJM03AudioDevice);
...
public:
	...
	ANCHORTIME							mAnchorTime;
	IOLock *							mTimeLock;
	UInt64								mRampUpdateCounter;
	UInt64								Xcopy[MAX_ANCHOR_ENTRIES];
	UInt64								Ycopy[MAX_ANCHOR_ENTRIES];

protected:
	DJM03ConfigurationDictionary *		mConfigDictionary;
	OSArray *							mControlGraph;
	OSArray *							mClockGraph;
    IORecursiveLock *					mInterfaceLock;
	IORecursiveLock *					mRegisteredEnginesMutex;
	IORecursiveLock *					mRegisteredStreamsMutex;
	Boolean								mTerminatingDriver;
	thread_call_t						mInitHardwareThread;
	thread_call_t						mRetryEQDownloadThread;
	thread_call_t						mProcessStatusInterruptThread;
	Boolean								mDeviceIsInMonoMode;
	OSArray *							mMonoControlsArray;		// this flag is set by DJM03AudioEngine::performFormatChange
	OSArray *							mRegisteredEngines;
	OSArray *							mRegisteredStreams;
	
	// These member variables are for anchored time stamp algorithm
	
	IOTimerEventSource *				mUpdateTimer;
	bool								mSingleSampleRateDevice;
	DJM03AudioEngine *				mFailingAudioEngine;
	DJM03AudioEngine *				mEngineToRestart;
...
public:
	...
	static	void			initHardwareThread (DJM03AudioDevice * aua, void * provider);	//  <rdar://problem/6686515>
	...
	virtual OSArray *		getOptimalClockPath ( DJM03AudioEngine * thisEngine, UInt8 streamInterface, UInt8 altSetting, UInt32 sampleRate, Boolean * otherEngineNeedSampleRateChange, UInt8 * clockPathGroupIndex = NULL );	// [rdar://4867843], <rdar://5811247>
	...
	virtual UInt32			determineClockPathUnitUsage ( DJM03AudioEngine * thisEngine, OSArray * thisClockPath );																			// [rdar://4867843]
	...
	virtual	IOReturn		registerEngineInfo (DJM03AudioEngine * usbAudioEngine);		//	<rdar://6420832>
	virtual	SInt32			getEngineInfoIndex (DJM03AudioEngine * inAudioEngine);
	...
	virtual	IOReturn		doPlaythroughSetup (DJM03AudioEngine * usbAudioEngine, OSArray * playThroughPaths, UInt8 interfaceNum, UInt8 altSettingNum, UInt8 inputTerminalID);	//	<rdar://5366067>
	...
	virtual	void			addVolumeControls (DJM03AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage);	// <rdar://6413207>
	virtual	void			addMuteControl (DJM03AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage);	// <rdar://6413207>
	...
	virtual DJM03ConfigurationDictionary *	getConfigDictionary (void) {return mConfigDictionary;}
	...
	static	IOReturn		deviceRequest (IOUSBDevRequest * request, DJM03AudioDevice * self, IOUSBCompletion * completion = 0);
	...
	virtual IOReturn		getAnchorFrameAndTimeStamp (UInt64 *frame, UInt64 *time);
	...
	virtual void			setShouldSyncSampleRates (DJM03AudioEngine * problemEngine) {mFailingAudioEngine = problemEngine;};
	// [rdar://5355808]
	virtual void			setShouldResetEngine ( DJM03AudioEngine * problemEngine ) { mEngineToRestart = problemEngine; };
	
	virtual	DJM03AudioEngine * otherEngine (DJM03AudioEngine * thisEngine);
	virtual IOReturn		getBothEngines (DJM03AudioEngine ** firstEngine, DJM03AudioEngine ** secondEngine);
	...
	// This is how DJM03Audio attempts device recovery.
