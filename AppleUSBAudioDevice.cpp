...
void DJM03AudioDevice::stop (IOService *provider) {
	...
	if (mUpdateTimer)
	{
		...
		debugIOLog ("? DJM03AudioDevice[%p]::stop () - Cancelling time stamp rate timer ...", this);
		mUpdateTimer->cancelTimeout ();
	...
	debugIOLog ("? DJM03AudioDevice[%p]::stop () - mControlInterface now closing ...", this);
	...
	debugIOLog("- DJM03AudioDevice[%p]::stop ()", this);
}

// <rdar://problem/7666699>
bool DJM03AudioDevice::allEnginesStopped ( void )
{
	bool result = TRUE;
...
bool DJM03AudioDevice::createAudioEngines (void) {
...
//	OSBoolean *	useSingleAudioEngine = NULL;
	...
	// Find USB interfaces.
	debugIOLog ("+ DJM03AudioDevice[%p]::createAudioEngines ()", this);

	FailIf (NULL == mControlInterface, Exit);
	
//	FailIf (kIOReturnSuccess != mConfigDictionary->getControlledStreamNumbers (&streamNumberArray, &numStreams), Exit);
//	FailIf (kIOReturnSuccess != mConfigDictionary->getNumStreamInterfaces (&numStreamInterfaces), Exit);
//	debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - streamNumberArray = %p, numStreams = %lu, numStreamInterfaces = %lu", this, streamNumberArray, numStreams, numStreamInterfaces);
...
//	useSingleAudioEngine = OSDynamicCast ( OSBoolean, mControlInterface->getProperty ( "UseSingleAudioEngine" ) );

	{
		...
		unsigned long long interfacerNumber = 1;
		streamInterfaceNumber = OSNumber::withNumber (interfacerNumber, 8);
...
/*	
	if (NULL != useSingleAudioEngine)
	{
		...
		{
			debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - Forced (via override kext) single audio engine", this);
			FailMessage ( false == ( result = createAudioEngine ( streamNumberArray ) ) );
		}
		else
		{
			debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - Forced (via override kext) separate audio engines", this);
	...
	{
		debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - Known devices that requires separate audio engines", this);
	...
	else
	{
		...
		{
			debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - Known single sample rate device", this);
			...
			while ( NULL != ( commonSampleRatesStreamList = findStreamsWithCommonSampleRates ( availableStreamsList ) ) )
			{
				...
				while ( NULL != ( compatibleEndpointsStreamList = findStreamsWithCompatibleEndpoints ( commonSampleRatesStreamList ) ) )
				{
					...
					{					
						...
						{
							debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - USB Audio 2.0 device", this);
					...
					{
						debugIOLog ("? DJM03AudioDevice[%p]::createAudioEngines () - USB Audio 1.0 device", this);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::createAudioEngines () - 0x%x", this, result);
	
	return result;
}

bool DJM03AudioDevice::activateAudioEngines (void) {
	
	bool	result = true;

	debugIOLog ("+ DJM03AudioDevice[%p]::activateAudioEngines ()", this);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::activateAudioEngines () - 0x%x", this, result);
	
	return result;
}

bool DJM03AudioDevice::createAudioEngine (OSArray * streamInterfaceNumberArray) {

	DJM03AudioEngine *	engine = NULL;
	bool					result = false;
	OSArray *				newStreamInterfaceArray = NULL;	//  <rdar://problem/6892754>
//	bool					formatSupported;				//  <rdar://problem/6892754>
	
	debugIOLog ( "+ DJM03AudioDevice[%p]::createAudioEngine ()", this );
	...
	debugIOLog ( "? DJM03AudioDevice[%p]::createAudioEngine () - create audio engine with stream interfaces:", this );
...
//			if ( ( kIOReturnSuccess != interfaceContainsSupportedFormat ( streamInterfaceNumber->unsigned8BitValue (), &formatSupported ) ) || formatSupported )
		...
		{
			debugIOLog ( "! DJM03AudioDevice[%p]::createAudioEngine () - could not get streamInterfaceNumber", this );
		}
	}
	
	debugIOLog ( "? DJM03AudioDevice[%p]::createAudioEngine () final new array count: %d", this, newStreamInterfaceArray->getCount() );
	...
	if ( newStreamInterfaceArray->getCount() > 0 )
	{
		...
		engine = OSTypeAlloc ( DJM03AudioEngine );
...
IOReturn DJM03AudioDevice::interfaceContainsSupportedFormat ( UInt8 streamInterface, bool *formatSupported ) {
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::interfaceContainsSupportedFormat ()", this );
	...
	*formatSupported = false;
	...
	for ( altSettingIndex = ( startAtZero ? 0 : 1 ); altSettingIndex < numAltSettings; altSettingIndex++ ) 
	{
		...
		{
			IOLog( "WARNING: DJM03Audio has detected an unsupported format type: Type II AC-3" );
		}
	}
	
Exit:
	
	debugIOLog ( "- DJM03AudioDevice[%p]::interfaceContainsSupportedFormat () streamInterface: %u returnCode: %d formatSupported: %u", this, streamInterface, returnCode, *formatSupported );
...
Boolean DJM03AudioDevice::ShouldUpdatePRAM (void) {
	...
	debugIOLog ("? DJM03AudioDevice[%p]::ShouldUpdatePRAM () - speaker's vendorID = 0x%x", this, vendorID);
	...
	if (TRUE == speakersGood) 
	{
		...
		if (kIOUSBVendorIDAppleComputer == vendorID) 
		{
			...
			if (OSDynamicCast (IOUSBRootHubDevice, usbRegEntry)) {
				...
				debugIOLog ("? DJM03AudioDevice[%p]::ShouldUpdatePRAM () - Directly connected to the root hub", this);
			} 
			else 
			{
	...
	if (FALSE == FindSoundNode ()) {
		result = TRUE;
	}

Exit:
	debugIOLog ("? DJM03AudioDevice[%p]::ShouldUpdatePRAM () - result = %d", this, result);
	return result;
}

Boolean DJM03AudioDevice::FindSoundNode (void) {
...
IOReturn DJM03AudioDevice::message (UInt32 type, IOService * provider, void * arg) {
	DJM03AudioEngine	*	currentEngine = NULL;
	OSDictionary *			currentEngineInfo = NULL;
	
	debugIOLog ("+ DJM03AudioDevice[%p]::message (0x%x, %p) - rc=%d", this, type, provider, getRetainCount ());
	...
	switch (type) 
	{
		...
		case kIOUSBMessagePortHasBeenReset:
			...
			debugIOLog ("? DJM03AudioDevice[%p]::message () - Flushing controls to the device.", this);
			flushAudioControls ();
			FailIf (NULL == mRegisteredEngines, Exit);
			debugIOLog ("? DJM03AudioDevice[%p]::message () - Resetting engines.", this);
			for (UInt8 engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++)
			{
				currentEngineInfo = OSDynamicCast (OSDictionary, mRegisteredEngines->getObject (engineIndex));
				FailIf (NULL == currentEngineInfo, Exit);
				...
				debugIOLog ("? DJM03AudioDevice[%p]::message () - Resetting engine %p...", this, currentEngine);
...
IOUSBInterface * DJM03AudioDevice::getUSBInterface ( UInt8 interfaceNumber ) {
...
const IOUSBConfigurationDescriptor * DJM03AudioDevice::getConfigurationDescriptor () {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::getConfigurationDescriptor ()", this);
	...
	debugIOLog ("? DJM03AudioDevice[%p]::getConfigurationDescriptor () - numConfigs = %d", this, numConfigs);
	
	if (1 < numConfigs)
	{
		usbDevice->GetConfiguration (&currentConfigValue);	

		debugIOLog ("? DJM03AudioDevice[%p]::getConfigurationDescriptor () - currentConfigValue = %d", this, currentConfigValue);
		...
		for (index = 0; index < numConfigs; index++)
		{
			...
			{
				debugIOLog ("? DJM03AudioDevice[%p]::getConfigurationDescriptor () - Found config %d (%p) at index %d", this, currentConfigValue, descriptor, index);
...
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::getConfigurationDescriptor () - configDescriptor=%p", this, configDescriptor);
	return configDescriptor;
}

// added for rdar://4168019 . Returns the device speed (high, full, or low).
UInt8 DJM03AudioDevice::getDeviceSpeed () {
	IOUSBDevice *			usbDevice;
	UInt8					speed = 0;
	...
	switch (speed)
	{
		case kUSBDeviceSpeedLow:
			debugIOLog ("? DJM03AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedLow", this);
			break;
		case kUSBDeviceSpeedFull:
			debugIOLog ("? DJM03AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedFull", this);
			break;
		case kUSBDeviceSpeedHigh:
			debugIOLog ("? DJM03AudioDevice[%p]::getDeviceSpeed () = kUSBDeviceSpeedHigh", this);
			break;
		default:
			debugIOLog ("? DJM03AudioDevice[%p]::getDeviceSpeed () = %d (UNKNOWN)", this, speed);
	}
	#endif
...
UInt64 DJM03AudioDevice::getUSBFrameNumber () {
	IOUSBDevice *			usbDevice;
...
UInt8 DJM03AudioDevice::getManufacturerStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
UInt8 DJM03AudioDevice::getProductStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
UInt8 DJM03AudioDevice::getSerialNumberStringIndex () {
	IOUSBDevice *			usbDevice;
	UInt8					stringIndex = 0;
...
IOReturn DJM03AudioDevice::getStringDescriptor(UInt8 index, char *buf, int maxLen, UInt16 lang) {
	IOUSBDevice *			usbDevice;
...
UInt16 DJM03AudioDevice::getVendorID () {
	IOUSBDevice *			usbDevice;
	UInt16					vendorID = 0;
...
UInt16 DJM03AudioDevice::getProductID () {
	IOUSBDevice *			usbDevice;
	UInt16					productID = 0;
...
OSNumber * DJM03AudioDevice::getLocationID () {
	IOUSBDevice *			usbDevice;
...
bool DJM03AudioDevice::detectSplitTransactions () {
	IOUSBDevice *			usbDevice;
	...
	{
		debugIOLog ("? DJM03AudioDevice::detectSplitTransactions () - This is a high speed device, so there are no split transactions.");
				...
				&&	(usbDevice))
	{
		...
		if (kUSBDeviceSpeedHigh == speed)
		{
			...
			debugIOLog ("? DJM03AudioDevice::detectSplitTransactions () = true");
...
Boolean DJM03AudioDevice::checkForUHCI () {
	...
	if (!strcmp (serviceName, "AppleUSBUHCI"))
	{
		...
		debugIOLog ("? DJM03AudioDevice::checkForUHCI () - UHCI connection detected!\n");
		result = TRUE;
	}
Exit:
	return result;
...
// <rdar://6420832>
IOReturn DJM03AudioDevice::registerEngineInfo (DJM03AudioEngine * usbAudioEngine) {
...
SInt32 DJM03AudioDevice::getEngineInfoIndex (DJM03AudioEngine * inAudioEngine) {
	OSDictionary *						engineInfo;
	DJM03AudioEngine *				usbAudioEngine;
	UInt16								engineIndex;
	SInt32								returnIndex;
	...
	if (mRegisteredEngines) 
	{
		for (engineIndex = 0; engineIndex < mRegisteredEngines->getCount (); engineIndex++) 
		{
			...
			if (engineInfo) 
			{
				usbAudioEngine = OSDynamicCast (DJM03AudioEngine, engineInfo->getObject (kEngine));
...
// <rdar://6420832>
IOReturn DJM03AudioDevice::registerStreamInfo (UInt8 interfaceNum, UInt8 altSettingNum) {
...
// <rdar://6420832>
SInt32 DJM03AudioDevice::getStreamInfoIndex (UInt8 interfaceNum) {
	OSDictionary *						streamInfo;
	OSNumber *							streamInterfaceNumber;
	UInt16								streamIndex;
	SInt32								returnIndex;
...
IOReturn DJM03AudioDevice::doControlStuff (IOAudioEngine *audioEngine, UInt8 interfaceNum, UInt8 altSettingNum) {
	DJM03AudioEngine *				usbAudioEngine;
	...
	debugIOLog ("+ DJM03AudioDevice::doControlStuff(0x%x, %d, %d)", audioEngine, interfaceNum, altSettingNum);
	result = kIOReturnError;
	inputSelector = NULL;
	done = FALSE;

    usbAudioEngine = OSDynamicCast (DJM03AudioEngine, audioEngine);
    FailIf (NULL == usbAudioEngine, Exit);
	FailIf (NULL == mControlInterface, Exit);
	debugIOLog ("? DJM03AudioDevice::doControlStuff () - This usbAudioEngine = %p", usbAudioEngine);
	...
	if (direction == kIOAudioStreamDirectionOutput) 
	{
		...
		debugIOLog("? DJM03AudioDevice::doControlStuff () - numConnectedOutputTerminals = %d inputTerminalID: %d", numConnectedOutputTerminals, inputTerminalID);
		...
		for (outputTerminalIndex = 0; outputTerminalIndex < numOutputTerminals && FALSE == done; outputTerminalIndex++) 
		{
			...
			if (terminalType != USB_STREAMING) 
			{
				...
				for (pathsToOutputTerminalN = 0; ( pathsToOutputTerminalN < numOutputTerminalArrays ) && !finished; pathsToOutputTerminalN++) 
				{
					...
					// <rdar://problem/6656791> DJM03Audio: Output controls not set up properly for stream interfaces
						...
						if ( ( inputUnitID == inputTerminalID ) && ( outputUnitID == outputTerminalID ) )	//	<rdar://6413207>
						{
							if ( !hasInitializedOutputControls )
							{
								...
								if (featureUnitID) 
								{
									...
									debugIOLog("? DJM03AudioDevice::doControlStuff () - Creating output gain controls");
								...
								if (featureUnitID) 
								{
									debugIOLog("? DJM03AudioDevice::doControlStuff () - Creating output mute controls");
			...
			for ( arrayOfPathsFromOutputTerminalIndex = 0; ( arrayOfPathsFromOutputTerminalIndex < numPathsFromOutputTerminals ) && !finished; arrayOfPathsFromOutputTerminalIndex++ )
			{
				...
				if (unitID == outputTerminalID) 
				{
					...
					if (numPathsFromOutputTerminal > 1 && numSelectorUnits) 
					{
						...
						for (unitIndexInPath = 1; unitIndexInPath < numUnitsInPath; unitIndexInPath++) 
						{
							...
							{							
								...
								{
									debugIOLog ( "? DJM03AudioDevice::doControlStuff () - Unable to get the selector setting. Defaulting to 1" );
									defaultSelectorSetting = 1;
								}
								debugIOLog ( "? DJM03AudioDevice::doControlStuff () - Default selector setting: %d", defaultSelectorSetting );
								...
								debugIOLog ( "? DJM03AudioDevice::doControlStuff () - Selected aPath = %p, pathIndex = %d, inputTerminalID = %d", aPath, pathIndex, inputTerminalID );
								...
								if (kIOReturnSuccess == setSelectorSetting (unitID, defaultSelectorSetting))	//	<rdar://6523676>
								{
									...
									if ( ( -1 != engineIndex ) && ( -1 != streamIndex ) ) 	//	<rdar://6420832>
									{
										...
										if (featureUnitID) 
										{
											...
											debugIOLog("? DJM03AudioDevice::doControlStuff () - Creating input gain controls");
											...
											debugIOLog("? DJM03AudioDevice::doControlStuff () - Creating input mute controls");
						...
						else 
						{
							...
							if (featureUnitID) 
							{
								...
								debugIOLog("? DJM03AudioDevice::doControlStuff () - Creating playthrough volume controls");
								addVolumeControls (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);	// <rdar://6413207>
								featureUnitID = 0;
							}
							...
							if (featureUnitID) 
							{
								...
								debugIOLog ( "? DJM03AudioDevice::doControlStuff () - Creating input mute control (no selectors)" );
								addMuteControl (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);		//	<rdar://6413207>
								featureUnitID = 0;
							}
						}
					...
					else 
					{
						...
						{
							// Create the playthrough volume controls
							debugIOLog ( "? DJM03AudioDevice::doControlStuff () - Creating input volume control (no selectors)" );
							addVolumeControls (usbAudioEngine, featureUnitID, inputTerminalID, interfaceNum, altSettingNum, kIOAudioControlUsageInput);		// <rdar://6413207>
							featureUnitID = 0;
						}
						featureUnitID = getBestFeatureUnitInPath (aPath, kIOAudioControlUsageInput, interfaceNum, altSettingNum, kMuteControl);
					...
					if (playThroughPaths) 
					{
						...
						debugIOLog("? DJM03AudioDevice::doControlStuff () - performing playthrough setup");
...
IOReturn DJM03AudioDevice::doPlaythroughSetup (DJM03AudioEngine * usbAudioEngine, OSArray * playThroughPaths, UInt8 interfaceNum, UInt8 altSettingNum, UInt8 inputTerminalID) {	//	<rdar://5366067>
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
...
//  <rdar://problem/6656791>
UInt32 DJM03AudioDevice::getPathIndexForSelectorSetting (OSArray * arrayOfPathsFromOutputTerminal, UInt32 pathsToOutputTerminalN, UInt32 graphPathIndex, UInt8 selectorUnitIndex, UInt8 selectorSetting) {
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
...
// <rdar://problem/6656791>
IOReturn DJM03AudioDevice::addSelectorSourcesToSelectorControl (IOAudioSelectorControl * theSelectorControl, OSArray * arrayOfPathsFromOutputTerminal, UInt32 pathsToOutputTerminalN, UInt32 graphPathIndex, UInt8 selectorIndex) {
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	OSString *							nameString;
	UInt32								selectorSourceIndex;
...
//	<rdar://6413207>
UInt8 DJM03AudioDevice::getDefaultOutputTerminalID (UInt8 inputTerminalID) {
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	UInt8								controlInterfaceNum;
...
//	<rdar://6413207>
UInt32 DJM03AudioDevice::getNumConnectedOutputTerminals (UInt8 inputTerminalID) {
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
	UInt8								controlInterfaceNum;
...
//	<rdar://6430836>
OSString * DJM03AudioDevice::getNameForTerminal (UInt8 terminalID, UInt8 direction) {
	...
	if ( ( kIOReturnSuccess == mConfigDictionary->getStringIndex ( &stringIndex, controlInterfaceNum, 0, terminalID ) ) &&
		 ...
		 ( kIOReturnSuccess == getStringDescriptor ( stringIndex, stringBuffer, kStringBufferSize ) ) )
	{						 
		debugIOLog ("? DJM03AudioDevice::getNameForTerminal () - terminalID = %d, stringIndex = %d, stringBuffer = %s", terminalID, stringIndex, stringBuffer);
...
OSString * DJM03AudioDevice::getNameForPath (OSArray * arrayOfPathsFromOutputTerminal, UInt32 * pathIndex, UInt8 startingPoint) {
	OSString *							theString;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
...
OSString * DJM03AudioDevice::getNameForMixerPath (OSArray * arrayOfPathsFromOutputTerminal, UInt32 * pathIndex, UInt8 startingPoint) {
	char								string[255];
	OSString *							theString;
	OSString *							tempString;
	OSArray *							aPath;
...
void DJM03AudioDevice::addVolumeControls (DJM03AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage) {	// <rdar://6413207>
	...
	debugIOLog ("+ DJM03AudioDevice::addVolumeControls (0x%x, %d, %d, %d, %d, %u)", usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage);
	FailIf (NULL == mControlInterface, Exit);
	...
	for (channelNum = 0; channelNum <= numControls; channelNum++) 
	{
		...
		{
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Creating volume controls for channel %d", this, channelNum);
			FailIf (kIOReturnSuccess != getCurVolume (featureUnitID, channelNum, &deviceCur), Error);
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - deviceCur = 0x%04x", this, deviceCur);
			FailIf (kIOReturnSuccess != getMinVolume (featureUnitID, channelNum, &deviceMin), Error);
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - deviceMin = 0x%04x", this, deviceMin);
			FailIf (kIOReturnSuccess != getMaxVolume (featureUnitID, channelNum, &deviceMax), Error);
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - deviceMax = 0x%04x", this, deviceMax);
			getVolumeResolution (featureUnitID, channelNum, &volRes);
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - volRes = 0x%04x", this, volRes);
			// [rdar://4511427] Need to check volRes for class compliance (Audio Spec 5.2.2.4.3.2).
			...
			if ( ( SInt16 ) kNegativeInfinity != deviceCur )
			{
				...
				if (		( deviceCur < deviceMin )
						||	( deviceCur > deviceMax ) )
				{
					debugIOLog ( "! DJM03AudioDevice::addVolumeControls () - deviceCur is not in volume range! Setting to deviceMin ..." );
					deviceCur = deviceMin;
				}
				debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Attempting to set volume to current volume ...", this);
				FailIf (kIOReturnSuccess != setCurVolume (featureUnitID, channelNum, HostToUSBWord(deviceCur)), Error);
				debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Volume was set successfully.", this);
			}

			if ( (SInt16) kNegativeInfinity == deviceMin ) 
			{
				debugIOLog ( "! DJM03AudioDevice[%p]::addVolumeControls () - device violates USB 2.0 audio spec Section 5.2.5.7.2! Setting deviceMin to 0x8001 ..." );
			...
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Creating control... [%d : %d : %d]", this, controlMin, controlCur, controlMax);
			...
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Created control %p", this, theLevelControl);
			...
			switch (usage) 
			{
				...
				case kIOAudioControlUsageOutput:
					...
					if (mDeviceIsInMonoMode)
					{
						...
						debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Added channel %d to mono controls array", this, channelNum);
						number->release ();
					}
					
					break;
				default:
					debugIOLog ("! DJM03AudioDevice[%p]::addVolumeControls () - Control %p has an unknown usage!", this, theLevelControl);
			}
			theLevelControl->release ();
		} 
		else 
		{
			debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Channel %d has no volume controls; skipping ...", this, channelNum);
		}
		goto NoError;
Error: 
		debugIOLog ("! DJM03AudioDevice[%p]::addVolumeControls () - Error creating controls for channel %d!", this, channelNum);
NoError:
		debugIOLog ("? DJM03AudioDevice[%p]::addVolumeControls () - Done with channel %d", this, channelNum);
	}
...
void DJM03AudioDevice::addMuteControl (DJM03AudioEngine * usbAudioEngine, UInt8 featureUnitID, UInt8 terminalID, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 usage)	// <rdar://6413207>
{
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::addMuteControl ( %p, %d, %d, %d, %d, 0%x )", this, usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage );
	FailIf ( NULL == mControlInterface, Exit );
	...
	for ( channelNum = 0; channelNum <= numControls; channelNum++ ) 
	{
		if ( mConfigDictionary->channelHasMuteControl ( controlInterfaceNum, 0, featureUnitID, channelNum ) )
		{
			...
			debugIOLog ( "? DJM03AudioDevice::addMuteControl () - channel %d, feature unit %d has current mute value 0x%x", channelNum, featureUnitID, deviceCur );
			...
			{
				debugIOLog ("! DJM03AudioDevice::addMuteControl () - forcing channel %d of this device to unmute in hardware.", channelNum );
				deviceCur = 0x0;
			}
			// <rdar://6413207> Add the terminal ID to the control ID so that it is unique. 
...
Exit:
	debugIOLog ( "- DJM03AudioDevice[%p]::addMuteControl ( %p, %d, %d, %d, %d, 0x%x )", this, usbAudioEngine, featureUnitID, terminalID, interfaceNum, altSettingNum, usage );
	return;
}
...
IOFixed DJM03AudioDevice::ConvertUSBVolumeTodB (SInt16 volume) {
...
IOReturn DJM03AudioDevice::getFeatureUnitRange (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, SubRange16 * target) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	struct {
...
IOReturn DJM03AudioDevice::getFeatureUnitSetting (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, SInt16 * target) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	UInt16								theSetting;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
...
IOReturn DJM03AudioDevice::setFeatureUnitSetting (UInt8 controlSelector, UInt8 unitID, UInt8 channelNumber, UInt8 requestType, UInt16 newValue, UInt16 newValueLen) {
    IOUSBDevRequestDesc					devReq;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
	IOReturn							result;
...
DeviceInactive:
	debugIOLog("? DJM03AudioDevice::setFeatureUnitSetting () - ERROR attempt to send a device request to an inactive device");
	goto Exit;
}

IOReturn DJM03AudioDevice::getCurMute (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM03AudioDevice::getCurVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM03AudioDevice::getMaxVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM03AudioDevice::getMinVolume (UInt8 unitID, UInt8 channelNumber, SInt16 * target) {
	IOReturn							result;
...
IOReturn DJM03AudioDevice::getVolumeResolution (UInt8 unitID, UInt8 channelNumber, UInt16 * target) {
	IOReturn							result;
...
IOReturn DJM03AudioDevice::setCurVolume (UInt8 unitID, UInt8 channelNumber, SInt16 volume) {
	return setFeatureUnitSetting (VOLUME_CONTROL, unitID, channelNumber, SET_CUR, volume, 2);
}

IOReturn DJM03AudioDevice::setCurMute (UInt8 unitID, UInt8 channelNumber, SInt16 mute) {
	return setFeatureUnitSetting (MUTE_CONTROL, unitID, channelNumber, SET_CUR, mute, 1);
}

IOReturn DJM03AudioDevice::controlChangedHandler (OSObject * target, IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
	DJM03AudioDevice *				self;

	result = kIOReturnError;

	self = OSDynamicCast (DJM03AudioDevice, target);
...
IOReturn DJM03AudioDevice::protectedControlChangedHandler (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
IOReturn DJM03AudioDevice::doSelectorControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn							result;
...
IOReturn DJM03AudioDevice::doVolumeControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	IOReturn							result;
	SInt16								newVolume;
	SInt16								deviceMin;
	SInt16								offset;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::doVolumeControlChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	     ...
	     || (FALSE == mDeviceIsInMonoMode))
	{
		...
		debugIOLog ("? DJM03AudioDevice[%p]::doVolumeControlChange () - Setting channel %d (unit %d) volume to 0x%x", this, channelNum, unitID, newVolume);
	...
	{	// mono output case
		...
		debugIOLog ("? DJM03AudioDevice[%p]::doVolumeControlChange () - Performing mono volume control change", this);
		for (i = 0; i < mMonoControlsArray->getCount (); i++)
		{
			...
			result = setCurVolume (unitID, channelNum, HostToUSBWord (newVolume));
			debugIOLog ("? DJM03AudioDevice[%p]::doVolumeControlChange () - Set volume for channel %d to 0x%x = %d", this, channelNum, newVolume, result);
		}
	}
	
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::doVolumeControlChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
	return result;
}

IOReturn DJM03AudioDevice::doToggleControlChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	IOReturn							result;
	UInt8								unitID;
	UInt8								channelNum;

	debugIOLog ("+ DJM03AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);

	unitID = audioControl->getControlID () & 0xFF;		// <rdar://6413207>
	channelNum = audioControl->getChannelID ();

	debugIOLog ("? DJM03AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x ) - unitID = %d, channelNum = %d", this, audioControl, oldValue, newValue, unitID, channelNum);

	result = setCurMute (unitID, channelNum, HostToUSBWord (newValue));

	debugIOLog ("- DJM03AudioDevice[%p]::doToggleControlChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);

	return result;
}

IOReturn DJM03AudioDevice::doPassThruSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM03AudioEngine *				usbAudioEngine;
	OSArray *							playThroughPaths;
	OSArray *							passThruVolControlsArray;
	OSArray *							passThruToggleControlsArray;	//	<rdar://5366067>
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);

	if ( oldValue != newValue )		//	<rdar://5366067>
	{
		pathIndex = (newValue & 0x000000FF);
		
		debugIOLog ("? DJM03AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x ) - controlID = 0x%x", this, audioControl, oldValue, newValue, audioControl->getControlID ());
...
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::doPassThruSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);
	return kIOReturnSuccess;
}

IOReturn DJM03AudioDevice::doInputSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM03AudioEngine *				usbAudioEngine;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::doInputSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
	...
	if ( oldValue != newValue )	//	<rdar://5366067>
	{
		...
		if ( !foundMixerUnit )
		{
			...
			if (playThroughPaths) 
			{
				debugIOLog("? DJM03AudioDevice::doInputSelectorChange () - performing playthrough setup");
...
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::doInputSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, result);
	return result;
}

//	<rdar://6413207>
IOReturn DJM03AudioDevice::doOutputSelectorChange (IOAudioControl * audioControl, SInt32 oldValue, SInt32 newValue) {
	DJM03AudioEngine *				usbAudioEngine;
	OSArray *							arrayOfPathsFromOutputTerminal;
	UInt32								numOutputTerminalArrays;
	UInt32								pathsToOutputTerminalN;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::doOutputSelectorChange( %p, 0x%x, 0x%x )", this, audioControl, oldValue, newValue);
...
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::doOutputSelectorChange( %p, 0x%x, 0x%x ) = 0x%x", this, audioControl, oldValue, newValue, kIOReturnSuccess);
	return kIOReturnSuccess;
}
...
OSArray * DJM03AudioDevice::getPlaythroughPaths (UInt8 inputTerminalID) {	//	<rdar://5366067>
	OSArray *							arrayOfPathsFromOutputTerminal;
	OSArray *							playThroughPaths;
	OSArray *							aPath;
	OSNumber *							theUnitIDNum;
...
UInt8 DJM03AudioDevice::getBestFeatureUnitInPath (OSArray * thePath, UInt32 direction, UInt8 interfaceNum, UInt8 altSettingNum, UInt32 controlTypeWanted) {
	OSNumber *							theUnitIDNum;
	...
	UInt8								channelNum;
	UInt8								numChannels;
	UInt8								selectorUnitIndex = 0;		//	<rdar://5366067>
	UInt8								mixerUnitIndex = 0;			//	<rdar://5366067>
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::getBestFeatureUnitInPath (%p, %lu, %d, %d, %lu)", this, thePath, direction, interfaceNum, altSettingNum, controlTypeWanted );
	featureUnitID = 0;
	FailIf (NULL == mControlInterface, Exit);
	FailIf (NULL == thePath, Exit);
	controlInterfaceNum = mControlInterface->GetInterfaceNumber ();
	...
	switch (direction) 
	{
		case kIOAudioControlUsagePassThru:
			...
			for (unitIndex = numUnitsInPath - 2; unitIndex > endingUnitIndex && !foundFeatureUnit; unitIndex--) 
			{
				...
				{
					debugIOLog ( "! DJM03AudioDevice::getBestFeatureUnitInPath () - something is wrong here!!!" );
				}
			} // for unitIndex
			break;
		case kIOAudioControlUsageInput:
		...
		default:
			...
			debugIOLog ("? DJM03AudioDevice::getBestFeatureUnitInPath () - kIOAudioControlUsageOutput ");
...
Exit:
	debugIOLog ( "- DJM03AudioDevice[%p]::getBestFeatureUnitInPath () = %d", this, featureUnitID );
	return featureUnitID;
}

UInt8 DJM03AudioDevice::pathsContaining (UInt8 unitID)
{
...
//	<rdar://5366067>
UInt8 DJM03AudioDevice::pathsContainingFeatureUnitButNotMixerUnit (UInt8 featureUnitID, UInt8 mixerUnitID)
{
	OSArray *	thisPathGroup = NULL;
	OSArray *	thisPath = NULL;
	OSNumber *  unitNumber = NULL;
...
UInt8 DJM03AudioDevice::getSelectorSetting (UInt8 selectorID) {
    IOUSBDevRequestDesc					devReq;
	IOReturn							result;
	UInt8								setting;
...
IOReturn DJM03AudioDevice::setSelectorSetting (UInt8 selectorID, UInt8 setting) {
    IOUSBDevRequestDesc					devReq;
	IOReturn							result;
	IOBufferMemoryDescriptor *			settingDesc = NULL;
...
void DJM03AudioDevice::setMonoState (Boolean state)
{
	mDeviceIsInMonoMode = state;
}

IOReturn DJM03AudioDevice::createControlsForInterface (IOAudioEngine *audioEngine, UInt8 interfaceNum, UInt8 altSettingNum) {
    DJM03AudioEngine *				usbAudioEngine;
    IOReturn							result;
    debugIOLog ("? DJM03AudioDevice[%p]::createControlsForInterface () - Interface %d alternate setting %d", this, interfaceNum, altSettingNum);
...
// [rdar://4867843]
OSArray * DJM03AudioDevice::getOptimalClockPath ( DJM03AudioEngine * thisEngine, UInt8 streamInterface, UInt8 altSetting, UInt32 sampleRate, Boolean * otherEngineNeedSampleRateChange, UInt8 * clockPathGroupIndex )	//	<rdar://5811247>
{
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::getOptimalClockPath ( %d, %d, %d )", this, streamInterface, altSetting, sampleRate );
	FailIf ( NULL == mConfigDictionary, Exit );
	FailIf ( NULL == mControlInterface, Exit );
	...
	debugIOLog (" ? DJM03AudioDevice::getOptimalClockPath () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSetting, clockSourceID );
...
Exit:
	...
	debugIOLog ( "- DJM03AudioDevice[%p]::getOptimalClockPath ( %d, %d, %d ) = %p", this, streamInterface, altSetting, sampleRate, optimalPathArray );
	return optimalPathArray;
}

OSArray * DJM03AudioDevice::getClockPathGroup ( UInt8 streamInterface, UInt8 altSetting, UInt8 * clockPathGroupIndex )	//	<rdar://5811247>
{
	OSNumber *						clockSourceIDNumber = NULL;
	OSArray *						pathGroupArray = NULL;
	OSArray *						pathArray = NULL;
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::getClockPathGroup ( %d, %d )", this, streamInterface, altSetting );
	FailIf ( NULL == mConfigDictionary, Exit );
	FailIf ( NULL == mControlInterface, Exit );
	...
	debugIOLog (" ? DJM03AudioDevice::getClockPathGroup () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSetting, clockSourceID );
...
Exit:
	...
	debugIOLog ( "- DJM03AudioDevice[%p]::getClockPathGroup ( %d, %d ) = %p", this, streamInterface, altSetting, pathGroupArray );
	return pathGroupArray;
}

//	<rdar://5811247>
OSArray * DJM03AudioDevice::getClockPathGroup ( UInt8 pathGroupIndex )
{
	OSArray *						pathGroupArray = NULL;
	
	debugIOLog ( "+ DJM03AudioDevice[%p]::getClockPathGroup ( %d )", this, pathGroupIndex );
...
Exit:
	...
	debugIOLog ( "- DJM03AudioDevice[%p]::getClockPathGroup ( %d ) = %p", this, pathGroupIndex, pathGroupArray );
	return pathGroupArray;
}

//	<rdar://5811247>
IOReturn DJM03AudioDevice::getClockSelectorIDAndPathIndex (UInt8 * selectorID, UInt8 * pathIndex, OSArray * clockPath) {
	IOReturn				result;
...
Boolean DJM03AudioDevice::supportSampleRateInClockPath ( OSArray * pathArray, UInt32 sampleRate )
{
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::supportSampleRateInClockPath ()", this );
...
Exit:
	debugIOLog ( "- DJM03AudioDevice[%p]::supportSampleRateInClockPath () = %d", this, sampleRateSupported );
	return sampleRateSupported;
}

UInt32 DJM03AudioDevice::determineClockPathUnitUsage ( DJM03AudioEngine * thisEngine, OSArray * thisClockPath )
{
	UInt32							usageCount = 0;
	SInt32							engineIndex;
	OSDictionary *					otherAudioEngineInfo = NULL;
	DJM03AudioEngine *			otherAudioEngine = NULL;

	debugIOLog ( "+ DJM03AudioDevice[%p]::determineClockPathUnitUsage ( %p, %p )", this, thisEngine, thisClockPath );
	...
	if ( NULL != mRegisteredEngines )
	{
		...
		{	
			...
			if ( NULL == ( otherAudioEngine = OSDynamicCast (DJM03AudioEngine, otherAudioEngineInfo->getObject (kEngine) ) ) )
			{
				break;
			}
			...
			if (thisEngine != otherAudioEngine)
			{
				if ( NULL != otherAudioEngine->mIOAudioStreamArray )
				{
					...
					{
						DJM03AudioStream * audioStream = OSDynamicCast ( DJM03AudioStream, otherAudioEngine->mIOAudioStreamArray->getObject (index) );
						
						if ( ( NULL != audioStream ) && ( NULL != audioStream->mActiveClockPath ) )
						{
							if ( clockPathCrossed ( thisClockPath, audioStream->mActiveClockPath ) )
							{
								usageCount++;
							}
						}
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::determineClockPathUnitUsage ( %p, %p ) = %d", this, thisEngine, thisClockPath, usageCount );
	return usageCount;
}

Boolean DJM03AudioDevice::clockPathCrossed ( OSArray * clockPathA, OSArray * clockPathB )
{
...
// [rdar://4867779]
IOReturn DJM03AudioDevice::addSampleRatesFromClockSpace ()
{
	IOReturn						result = kIOReturnError;
	OSNumber *						clockSourceIDNumber = NULL;
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::addSampleRatesFromClockSpace ()", this );
	FailIf ( NULL == mControlInterface, Exit );		// <rdar://7085810>
	FailIf ( NULL == mConfigDictionary, Exit );
	...
	for ( UInt8 streamInterfaceIndex = 0; streamInterfaceIndex < streamInterfaceNumbers->getCount (); streamInterfaceIndex++ )
	{
					...
					altSettingIndex++ )
		{
			...
			debugIOLog (" ? DJM03AudioDevice::addSampleRatesFromClockSpace () - interface %d, alt setting %d, clock source ID %d", streamInterface, altSettingIndex, clockSourceID );
			// Find the path group that begins with the clockSourceID.
			for ( UInt8 pathGroupIndex = 0; pathGroupIndex < mClockGraph->getCount (); pathGroupIndex++ )
			{
				FailIf ( NULL == ( pathGroupArray = OSDynamicCast ( OSArray, mClockGraph->getObject ( pathGroupIndex ) ) ), Exit );
...
Exit:
	debugIOLog ( "- DJM03AudioDevice[%p]::addSampleRatesFromClockSpace () = 0x%x", this, result );
	return result;
}

IOReturn DJM03AudioDevice::addSampleRatesFromClockPath ( OSArray * path, UInt8 streamInterface, UInt8 altSetting )
{
	IOReturn						result = kIOReturnError;
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::addSampleRatesFromClockPath ( %p, %d, %d )", this, path, streamInterface, altSetting );
...
Exit:
	...
	debugIOLog ( "- DJM03AudioDevice[%p]::addSampleRatesFromClockPath ( %p, %d, %d ) = 0x%x", this, path, streamInterface, altSetting, result );
	return result;
}

IOReturn DJM03AudioDevice::getClockSourceSampleRates ( OSArray ** sampleRates, UInt8 clockSource )
{
	IOReturn						result = kIOReturnError;
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::getClockSourceSampleRates ( %p, %d )", this, sampleRates, clockSource );
	FailIf ( NULL == mConfigDictionary, Exit );
	...
	* sampleRates = NULL;
		 ...
		 mConfigDictionary->clockSourceHasFrequencyControl ( mControlInterface->GetInterfaceNumber (), 0, clockSource, false ) )
	{
		...
		for ( UInt8 sampleRangeIndex = 0; sampleRangeIndex < numSampleRanges; sampleRangeIndex++ )
		{
			...
			if ( ! subRangeIsValid )
			{
				debugIOLog ( "! DJM03AudioDevice[%p]::getClockSourceSampleRates () - invalid subrange, skipping ...", this );
...
Exit:
	debugIOLog ( "- DJM03AudioDevice[%p]::getClockSourceSampleRates ( %p, %d ) = 0x%x", this, sampleRates, clockSource, result );
	return result;
}

OSArray * DJM03AudioDevice::buildClockGraph ( UInt8 controlInterfaceNum )
{
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::buildClockGraph ( %d )", this, controlInterfaceNum );
	...
	for ( UInt8 clockIndex = 0; clockIndex < terminalClockEntities->getCount (); clockIndex++) 
	{
		...
		debugIOLog ( "? DJM03AudioDevice[%p]::buildClockGraph () - Building clock paths from ID %d", this, clockID ); 
	...
	debugIOLog ( "? DJM03AudioDevice[%p]::buildClockGraph ( %d ) - Displaying graph ...", this, controlInterfaceNum, allClockPaths->getCount() );
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::buildClockGraph (%d) = %p", this, controlInterfaceNum, allClockPaths );
	return allClockPaths;
}

OSArray * DJM03AudioDevice::buildClockPath ( UInt8 controlInterfaceNum, UInt8 startingUnitID, OSArray * allPaths, OSArray * startingPath ) 
{
	...
	debugIOLog ( "+ DJM03AudioDevice[%p]::buildClockPath ( %d, %d, %p, %p )", controlInterfaceNum, startingUnitID, allPaths, startingPath );
	FailIf ( NULL == mConfigDictionary, Exit );
...
Exit:
	...
	debugIOLog ( "- DJM03AudioDevice[%p]::buildClockPath () = %p", curPath );
	return curPath;
}

OSArray * DJM03AudioDevice::BuildConnectionGraph (UInt8 controlInterfaceNum) 
{
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::BuildConnectionGraph (%d)", this, controlInterfaceNum);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::BuildConnectionGraph (%d) = %p", this, controlInterfaceNum, allOutputTerminalPaths);
	return allOutputTerminalPaths;
}

OSArray * DJM03AudioDevice::BuildPath (UInt8 controlInterfaceNum, UInt8 startingUnitID, OSArray * allPaths, OSArray * startingPath) {
	OSArray *						curPath = NULL;
...
char * DJM03AudioDevice::TerminalTypeString (UInt16 terminalType) 
{
...
//	<rdar://5811247>
char * DJM03AudioDevice::ClockTypeString (UInt8 clockType) 
{
...
IOReturn DJM03AudioDevice::deviceRequest (IOUSBDevRequestDesc * request, IOUSBCompletion * completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
	...
	debugIOLog ("? DJM03AudioDevice[%p]::deviceRequest (%p, %p) = %lx", this, request, completion, result);
	#endif
Exit:
	return result;
}

IOReturn DJM03AudioDevice::deviceRequest (IOUSBDevRequest * request, IOUSBCompletion * completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
...
IOReturn DJM03AudioDevice::deviceRequest (IOUSBDevRequest *request, DJM03AudioDevice * self, IOUSBCompletion *completion) {
	IOReturn						result;
	UInt32							timeout;
	Boolean							done;
	...
	#if LOGDEVICEREQUESTS
	debugIOLog ("? DJM03AudioDevice[%p]::deviceRequest (%p, %p) = %lx", self, request, completion, result);
	#endif
Exit:
	return result;
}

bool DJM03AudioDevice::willTerminate (IOService * provider, IOOptionBits options) {
	debugIOLog ("+ DJM03AudioDevice[%p]::willTerminate (%p)", this, provider);
	...
	debugIOLog ("- DJM03AudioDevice[%p]::willTerminate ()", this);

	return super::willTerminate (provider, options);
}

void DJM03AudioDevice::setConfigurationApp (const char *bundleID) {
	setConfigurationApplicationBundle (bundleID);
}

#ifdef DEBUG

void DJM03AudioDevice::retain() const
{
//    debugIOLog("DJM03AudioDevice(%p)::retain() - rc=%d", this, getRetainCount());
    super::retain();
}

void DJM03AudioDevice::release() const
{
//    debugIOLog("DJM03AudioDevice(%p)::release() - rc=%d", this, getRetainCount());
	super::release();
}
...
bool DJM03AudioDevice::matchPropertyTable(OSDictionary * table, SInt32 *score)
{
	bool		returnValue = false;
	OSObject *	deviceName;
	
	// debugIOLog ("+DJM03AudioDevice[%p]::matchPropertyTable (%p, %p)", this, table, score);
	...
	if (    (deviceName)
		 && (returnValue))
	{
		debugIOLog ("? DJM03AudioDevice[%p]::matchPropertyTable (%p, %p) = %d (custom dictionary match)", 
...
#if DEBUGANCHORS
void DJM03AudioDevice::accumulateAnchor (UInt64 anchorFrame, AbsoluteTime timeStamp)
{
	UInt32		i;
	...
	if (mAnchorFrames[(kAnchorsToAccumulate - 1)] != 0ull)
	{
		...
		debugIOLog ("? DJM03AudioDevice::accumulateAnchor () - Frame # %d accumulated.", kAnchorsToAccumulate);
		for (i = 0; i < kAnchorsToAccumulate ; i++)
		{
...
IOReturn DJM03AudioDevice::getAnchorFrameAndTimeStamp (UInt64 *frame, UInt64 *time) {
	UInt64			finishTime;
	UInt64			offset;
	UInt64			curTime;
	UInt64			prevTime;
	...
	FailIf (finishTime < curTime, Exit);		// if we timed out
	diffAbs = curTime;
	diffAbs -= prevTime;
...
UInt64 DJM03AudioDevice::getWallTimeInNanos (void)
{
	UInt64			time;
	UInt64			timeInNanos;
...
#if DEBUGANCHORS	
	debugIOLog ("? DJM03AudioDevice::updateAnchorTime () - index: %u X: %llu Y: %llu", anchorTime->index, X, Y);
#endif	
...
UInt64 DJM03AudioDevice::getTimeForFrameNumber ( UInt64 frameNumber )
{
	UInt64 result = 0;
...
void DJM03AudioDevice::updateUSBCycleTime ( void )
{
	UInt64			timeStamp;
	UInt64			frameNumber;
...
#if DEBUGTIMESTAMPS	 
		debugIOLog ("? DJM03AudioDevice::updateUSBCycleTime() mWallTimePerUSBCycle = %llu frames elapsed: %llu\n", mWallTimePerUSBCycle, frameNumber - lastAnchorFrame() );
#endif				
...
// <rdar://problem/7666699>
void DJM03AudioDevice::applyOffsetAmountToFilter ( void )
{
	UInt64			timeOffset = 0;
...
#if DEBUGANCHORS						
			debugIOLog ("? DJM03AudioDevice::applyOffsetAmountToFilter timeOffset: %llu framesElapsed: %llu currentFrame: %llu predictedTime: %llu actualTime: %llu", 
...
void DJM03AudioDevice::calculateOffset ( void )
{
	debugIOLog ( "+ DJM03AudioDevice::calculateOffset (%p)", this );
...
void DJM03AudioDevice::resetRateTimer ()
{
	mWallTimePerUSBCycle = 1000000ull * kWallTimeExtraPrecision;
...
// <rdar://problem/7666699>
UInt64 DJM03AudioDevice::lastAnchorFrame ( void )
{
	return ( mAnchorTime.index ? mAnchorTime.X[mAnchorTime.index - 1] : mAnchorTime.X[MAX_ANCHOR_ENTRIES - 1] );
}

void DJM03AudioDevice::TimerAction (OSObject * owner, IOTimerEventSource * sender) 
{
	DJM03AudioDevice *	self;
	
	#if DEBUGTIMER
		debugIOLog ("+ DJM03AudioDevice::TimerAction (%p, %p)", owner, sender);
	#endif
	FailIf (NULL == owner, Exit);
	self = (DJM03AudioDevice *) owner;
	FailIf (NULL == self, Exit);
	self->doTimerAction ( sender );
Exit:
	#if DEBUGTIMER
		debugIOLog ("- DJM03AudioDevice::TimerAction ()");
	#endif
	return;
}

void DJM03AudioDevice::doTimerAction (IOTimerEventSource * timer) 
{
	...
	#if DEBUGTIMER
		debugIOLog ("+ DJM03AudioDevice::doTimerAction (%p)", timer);
	#endif
	...
	if ( ++mTimerCallCount >= ( kRefreshInterval / curRefreshInterval ) )		// Always perform these items at kRefreshInterval
	{
		...
		{
			debugIOLog ("! DJM03AudioDevice[%p]::doTimerAction () - Detected failing audio engine (%p)! Performing emergency format change.", this, mFailingAudioEngine);
		...
		{
			// [rdar://5355808] If DJM03AudioEngine::CoalesceInputSamples () gets more than a USB frame's worth of samples behind, we need to restart the engine.
			debugIOLog ( "! DJM03AudioDevice[%p]::doTimerAction () - Restarting engine %p", this, mEngineToRestart );
...
Exit:
	#if DEBUGTIMER
		debugIOLog ("- DJM03AudioDevice::doTimerAction ()");
	#endif
	return;
}
...
UInt32 DJM03AudioDevice::formatChangeController (IOAudioEngine *audioEngine, IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
	DJM03AudioEngine *			thisAudioEngine = NULL;
	DJM03AudioEngine *			otherAudioEngine = NULL;
	DJM03AudioStream *			thisStream = NULL;
	DJM03AudioStream *			otherStream = NULL;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::formatChangeController (%p, %p, %p, %p)", this, audioEngine, audioStream, newFormat, newSampleRate);
	FailIf (NULL == mControlInterface, Exit);
	
	thisAudioEngine = (DJM03AudioEngine *) audioEngine;
	...
	if (mustMatchFormats)
	{
		debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - Attempting to match this format with the format for the other stream interface.", this);
		...
		debugIOLog ("\n");
		debugIOLog ("-------------------- BEFORE --------------------");
		debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - engine %p (interface %d, alternateSetting %d) info:", this, thisAudioEngine, thisStream->mInterfaceNumber, thisStream->mAlternateSettingID);
		...
		debugIOLog ("\n");
		debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - engine %p (interface %d, alternateSetting %d) info:", this, otherAudioEngine, otherStream->mInterfaceNumber, otherStream->mAlternateSettingID);
		...
		debugIOLog ("\n");
		debugIOLog (" DJM03AudioDevice[%p]::formatChangeController () - newFormat = %p", this, newFormat);
		if (newFormat)
		{
		...
		debugIOLog ( " DJM03AudioDevice[%p]::formatChangeController () - newSampleRate = %p", this, newSampleRate );
		if ( newSampleRate )
		{
		...
		if ( ( false == mSingleSampleRateDevice ) && ( false == otherEngineNeedSampleRateChange ) )
		{
			...
			if (kIOReturnSuccess == mConfigDictionary->getAltSettingWithSettings (&altSetting, thisStream->mInterfaceNumber, thisFormat->fNumChannels, thisFormat->fBitDepth, otherSampleRate->whole))
			{
				...
				{
					debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - This engine (%p) sample rate changed successfully to %lu.", this, thisAudioEngine, otherSampleRate->whole);
			...
			else if (kIOReturnSuccess == mConfigDictionary->getAltSettingWithSettings (&altSetting, otherStream->mInterfaceNumber, otherFormat->fNumChannels, otherFormat->fBitDepth, thisSampleRate->whole))
			{
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - Other engine (%p) sample rate changed succsesfully to %lu.", this, otherAudioEngine, thisSampleRate->whole);
					result = kAUAFormatChangeForced;
			...
			else
			{
				...
				debugIOLog ("! DJM03AudioDevice[%p]::formatChangeController () - Restoring both engines to their default settings.", this);
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - This engine (%p) restored to default settings succsesfully.", this, thisAudioEngine);
					result = kAUAFormatChangeForced;
				...
				if (kIOReturnSuccess == formatChangeReturnCode)
				{
					debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - Other engine (%p) restored to default settings succsesfully.", this, otherAudioEngine);
		...
		else
		{
					...
					&&	(newSampleRate->whole != otherSampleRate->whole))
			{
				...
				{
					// Fail this request.
					debugIOLog ("! DJM03AudioDevice[%p]::formatChangeController () - Other audio engine (%p) does not support sample rate %lu at %d bit %d channel(s). Failing.", 
	...
	{
		debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - Attempting normal format change request.", this);
	...
	if (kIOReturnSuccess != formatChangeReturnCode)
	{
		...
		debugIOLog ("! DJM03AudioDevice[%p]::formatChangeController () - This format change failed with error 0x%x.", this, result);
	}
	else
	{
		debugIOLog ("? DJM03AudioDevice[%p]::formatChangeController () - This format change was successful.", this);
		if	(		(kAUAFormatChangeNormal != result)
				&&	(kAUAFormatChangeForced != result))
		{
			debugIOLog ("! DJM03AudioDevice[%p]::formatChangeController () - Forced format change failed.", this);
			result = kAUAFormatChangeForceFailure;
		}
	}
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::formatChangeController (%p, %p, %p, %p) = 0x%x", this, audioEngine, audioStream, newFormat, newSampleRate, result);
	return result;
}

DJM03AudioEngine * DJM03AudioDevice::otherEngine (DJM03AudioEngine * thisEngine)
{
	SInt32	engineIndex;
	...
	otherAudioEngine = OSDynamicCast (DJM03AudioEngine, otherAudioEngineInfo->getObject (kEngine));
	FailIf (NULL == otherAudioEngine, Exit);
Exit:
	return otherAudioEngine;
}

IOReturn DJM03AudioDevice::getBothEngines (DJM03AudioEngine ** firstEngine, DJM03AudioEngine ** secondEngine)
{
	SInt32			engineIndex;
	...
	*firstEngine = OSDynamicCast (DJM03AudioEngine, firstAudioEngineInfo->getObject (kEngine));
...
void DJM03AudioDevice::attemptDeviceRecovery ()
{
	debugIOLog ("+ DJM03AudioDevice[%p]::attemptDeviceRecovery ()", this);
	
	FailIf (NULL == mControlInterface, Exit);
	/*
	debugIOLog ("? DJM03AudioDevice[%p]::attemptDeviceRecovery () - Issuing device reset.", this);
...
IOReturn DJM03AudioDevice::getClockSetting (UInt8 controlSelector, UInt8 unitID, UInt8 requestType, void * target, UInt16 length) {
    IOReturn							result;
	IOUSBDevRequestDesc					devReq;
	UInt16								theSetting;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
...
IOReturn DJM03AudioDevice::setClockSetting (UInt8 controlSelector, UInt8 unitID, UInt8 requestType, void * target, UInt16 length) {
    IOUSBDevRequestDesc					devReq;
	IOBufferMemoryDescriptor *			theSettingDesc = NULL;
	IOReturn							result;
...
DeviceInactive:
	debugIOLog("? DJM03AudioDevice::setClockSourceSetting () - ERROR attempt to send a device request to an inactive device");
	goto Exit;
}

IOReturn DJM03AudioDevice::getNumClockSourceSamplingFrequencySubRanges (UInt8 unitID, UInt16 * numSubRanges) {
	struct {
	UInt16								wNumSubRanges;
...
IOReturn DJM03AudioDevice::getIndexedClockSourceSamplingFrequencySubRange (UInt8 unitID, SubRange32 * subRange, UInt16 subRangeIndex) {
	struct {
	UInt16								wNumSubRanges;
	SubRange32							SubRanges[1];
	}									rangeParameterBlock;
...
IOReturn DJM03AudioDevice::getCurClockSourceSamplingFrequency (UInt8 unitID, UInt32 * samplingFrequency, bool * validity) {
...
IOReturn DJM03AudioDevice::setCurClockSourceSamplingFrequency (UInt8 unitID, UInt32 samplingFrequency) {
	UInt32								frequency;
...
IOReturn DJM03AudioDevice::getCurClockSelector (UInt8 unitID, UInt8 * selector) {
	return getClockSetting (USBAUDIO_0200::CX_CLOCK_SELECTOR_CONTROL, unitID, USBAUDIO_0200::CUR, selector, 1);
}

IOReturn DJM03AudioDevice::setCurClockSelector (UInt8 unitID, UInt8 selector) {
	return setClockSetting (USBAUDIO_0200::CX_CLOCK_SELECTOR_CONTROL, unitID, USBAUDIO_0200::CUR, &selector, 1);
}

IOReturn DJM03AudioDevice::getCurClockMultiplier (UInt8 unitID, UInt16 * numerator, UInt16 * denominator) {
...
IOReturn DJM03AudioDevice::getNumSampleRatesForClockPath (UInt8 * numSampleRates, OSArray * clockPath) {
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt8								clockID;
	UInt8								subType;
...
IOReturn DJM03AudioDevice::getIndexedSampleRatesForClockPath (SubRange32 * sampleRates, OSArray * clockPath, UInt32 rangeIndex) {
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
...
IOReturn DJM03AudioDevice::getClockPathCurSampleRate (UInt32 * sampleRate, Boolean * validity, Boolean * isReadOnly, OSArray * clockPath) {		//	<rdar://6945472>
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::getClockPathCurSampleRate", this);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::getClockPathCurSampleRate (%d, %d) = %d", this, clockRate, clockValidity, result);

	return result;
}

IOReturn DJM03AudioDevice::setClockPathCurSampleRate (UInt32 sampleRate, OSArray * clockPath, bool failIfReadOnly ) {	//	<rdar://6945472>
	OSObject *							arrayObject = NULL;
	OSNumber *							arrayNumber = NULL;
	UInt32								clockIndex;
	UInt8								clockID;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::setClockPathCurSampleRate (%d)", this, sampleRate);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::setClockPathCurSampleRate (%d) = %d", this, sampleRate, result);
...
OSArray * DJM03AudioDevice::findStreamsWithCommonSampleRates (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::findStreamsWithCommonSampleRates (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM03AudioDevice[%p]::findStreamsWithCommonSampleRates (%p) - Found streams:", this, availableStreamList);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::findStreamsWithCommonSampleRates (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's sample rates are common with at least one other stream in the stream list.
bool DJM03AudioDevice::isSampleRateCommonWithAtLeastOneStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM03AudioDevice::isSampleRateCommonWithAllStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM03AudioDevice::streamsHaveCommonSampleRates (OSNumber * streamInterfaceNumberA, OSNumber * streamInterfaceNumberB) {
	
	bool		result = false;
	OSArray *	sampleRatesA = NULL;
	OSArray *	sampleRatesB = NULL;
	 
	debugIOLog ("+ DJM03AudioDevice[%p]::streamsHaveCommonSampleRates (%p, %p)", this, streamInterfaceNumberA, streamInterfaceNumberB);

	FailIf ( NULL == streamInterfaceNumberA, Exit );
	FailIf ( NULL == streamInterfaceNumberB, Exit );

	debugIOLog ("? DJM03AudioDevice[%p]::streamsHaveCommonSampleRates (%d, %d)", this, streamInterfaceNumberA->unsigned8BitValue (), streamInterfaceNumberB->unsigned8BitValue ());
...
Exit:
	debugIOLog ("+ DJM03AudioDevice[%p]::streamsHaveCommonClocks (%p, %p) - result = %d", this, streamInterfaceNumberA, streamInterfaceNumberB, result);

	return result;
}

//	<rdar://5131786>	Get the sample rates supported by a stream interface.
OSArray * DJM03AudioDevice::getSampleRatesFromStreamInterface (OSNumber * streamInterfaceNumber) {
...
void DJM03AudioDevice::mergeSampleRates (OSArray * thisArray, OSArray * otherArray)
{
...
bool DJM03AudioDevice::compareSampleRates (OSArray * sampleRatesA, OSArray * sampleRatesB)
{
...
//	The caller is responsible to release the returned stream array.
OSArray * DJM03AudioDevice::findStreamsWithCompatibleEndpoints (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM03AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p) - Found streams:", this, availableStreamList);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::findStreamsWithCompatibleEndpoints (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Find streams with the specified direction & sync type in the available list.
OSArray * DJM03AudioDevice::findStreamsWithDirectionAndSyncType (OSArray * availableStreamList, UInt8 direction, UInt8 syncType ) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d)", this, availableStreamList, direction, syncType);
	...
	if ( NULL != result )
	{
		debugIOLog ("- DJM03AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d) - Found streams:", this, availableStreamList, direction, syncType);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::findStreamsWithDirectionAndSyncType (%p, %d, %d) - result = %p", this, availableStreamList, direction, syncType, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's endpoints are compatible with the specified direction & sync type.
bool DJM03AudioDevice::streamEndpointsHaveSpecifiedDirectionAndSyncType ( OSNumber * streamInterfaceNumber, UInt8 endpointDirection, UInt8 endpointSyncType ) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%p, %d, %d)", this, streamInterfaceNumber, endpointDirection, endpointSyncType);

	FailIf ( NULL == streamInterfaceNumber, Exit );

	interfaceNum = streamInterfaceNumber->unsigned8BitValue ();
	
	debugIOLog ("? DJM03AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%d, %d, %d)", this, interfaceNum, endpointDirection, endpointSyncType);
...
Exit:
	...
	debugIOLog ("- DJM03AudioDevice[%p]::streamEndpointsHaveSpecifiedDirectionAndSyncType (%p, %d, %d) - result = %d", this, streamInterfaceNumber, endpointDirection, endpointSyncType, result);
...
//	The caller is responsible to release the returned stream array.
OSArray * DJM03AudioDevice::findStreamsWithCommonClocks (OSArray * availableStreamList) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::findStreamsWithCommonClocks (%p)", this, availableStreamList);
	...
	if ( NULL != result )
	{
		debugIOLog ("? DJM03AudioDevice[%p]::findStreamsWithCommonClocks (%p) - Found streams:", this, availableStreamList);
		for ( UInt32 streamInterfaceIndex = 0; streamInterfaceIndex < result->getCount (); streamInterfaceIndex++ ) 
		{
			OSNumber * streamInterfaceNumber = OSDynamicCast (OSNumber, result->getObject ( streamInterfaceIndex ) );
			FailMessage ( NULL == streamInterfaceNumber );
...
Exit:	
	...
	debugIOLog ("- DJM03AudioDevice[%p]::findStreamsWithCommonClocks (%p) - result = %p", this, availableStreamList, result);

	return result;
}

//	<rdar://5131786>	Determine if the stream's clocks are common with at least one other stream in the stream list.
bool DJM03AudioDevice::isClockCommonWithAtLeastOneStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM03AudioDevice::isClockCommonWithAllStreamsInList (OSNumber * refStreamInterfaceNumber, OSArray * streamInterfaceNumberList) {
...
bool DJM03AudioDevice::streamsHaveCommonClocks (OSNumber * streamInterfaceNumberA, OSNumber * streamInterfaceNumberB) {
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::streamsHaveCommonClocks (%p, %p)", this, streamInterfaceNumberA, streamInterfaceNumberB);

	FailIf ( NULL == streamInterfaceNumberA, Exit );
	FailIf ( NULL == streamInterfaceNumberB, Exit );
	
	debugIOLog ("? DJM03AudioDevice[%p]::streamsHaveCommonClocks (%d, %d)", this, streamInterfaceNumberA->unsigned8BitValue (), streamInterfaceNumberB->unsigned8BitValue ());
...
bool DJM03AudioDevice::isClockPathCrossed (OSNumber * streamInterfaceNumber, OSArray * otherClockPath, UInt32 sampleRate)
{
	UInt8		interfaceNum;
	UInt8		numAltSettings = 0;
	OSArray *	clockPathGroup;
	...
	debugIOLog ("+ DJM03AudioDevice[%p]::isClockPathCrossed (%p, %p, %lu)", this, streamInterfaceNumber, otherClockPath, sampleRate);
...
Exit:
	debugIOLog ("- DJM03AudioDevice[%p]::isClockPathCrossed (%p, %p, %lu) - result = %d", this, streamInterfaceNumber, otherClockPath, sampleRate, result);
